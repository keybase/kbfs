...
func TestBasicMDUpdate(t *testing.T) {
	...
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode1, "a", false, false)
	if err != nil {
...
func testMultipleMDUpdates(t *testing.T, unembedChanges bool) {
	...
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode1, "c", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestUnmergedAfterRestart(t *testing.T) {
	...
	kbfsOps1 := config1.KBFSOps()
	fileNode1, _, err := kbfsOps1.CreateFile(ctx, rootNode1, "a", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestBasicCRNoConflict(t *testing.T) {
	...
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode1, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}

	// User 2 makes a new different file
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2, "c", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRFileConflictWithMoreUpdatesFromOneUser(t *testing.T) {
	...
	fileB1, _, err := kbfsOps1.CreateFile(ctx, dirA1, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestBasicCRFileCreateUnmergedWriteConflict(t *testing.T) {
	...
	// User 1 creates a file
	_, _, err = kbfsOps1.CreateFile(ctx, dirA1, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}

	// User 2 creates the same file, and writes to it.
	fileB2, _, err := kbfsOps2.CreateFile(ctx, dirA2, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	data2 := []byte{5, 4, 3, 2, 1}
...
func TestCRDouble(t *testing.T) {
	...
	kbfsOps1 := config1.KBFSOps()
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode, "a", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	...
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}

	// User 2 makes a couple revisions
	fileNodeC, _, err := kbfsOps2.CreateFile(ctx, rootNode2, "c", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	...
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2, "e", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2, "f", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	...
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode, "g", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}

	// User 2 makes a couple unmerged revisions
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2, "h", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2, "i", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRSyncParallelBlocksErrorCleanup(t *testing.T) {
	...
	fileNodeB, _, err := kbfsOps2.CreateFile(ctx, rootNode2, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRCanceledAfterNewOperation(t *testing.T) {
	...
	rootNode := GetRootNodeOrBust(t, config1, name, false)
	kbfsOps1 := config1.KBFSOps()
	aNode1, _, err := kbfsOps1.CreateFile(ctx, rootNode, "a", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	data := []byte{1, 2, 3, 4, 5}
	...
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	c <- struct{}{}
