...
func totalBlockRefs(m map[IFCERFTBlockID]map[IFCERFTBlockRefNonce]blockRefLocalStatus) int {
	n := 0
	for _, refs := range m {
		n += len(refs)
	}
...
func TestQuotaReclamationUnembedded(t *testing.T) {
	...
	if md.data.cachedChanges.Info.IFCERFTBlockPointer == zeroPtr {
...
func TestQuotaReclamationDeletedBlocks(t *testing.T) {
	...
	aNode1, _, err := kbfsOps1.CreateFile(ctx, rootNode1, "a", false, IFCERFTNoEXCL)
	if err != nil {
		t.Fatalf("Couldn't create dir: %v", err)
	}
	...
	for _, name := range []string{"b", "c"} {
		node, _, err := kbfsOps1.CreateFile(ctx, rootNode1, name, false, IFCERFTNoEXCL)
		if err != nil {
			t.Fatalf("Couldn't create dir: %v", err)
		}
	...
	dNode, _, err := kbfsOps2.CreateFile(ctx, rootNode2, "d", false, IFCERFTNoEXCL)
	if err != nil {
	...
	eNode, _, err := kbfsOps2.CreateFile(ctx, rootNode2, "e", false, IFCERFTNoEXCL)
	if err != nil {
		t.Fatalf("Couldn't create dir: %v", err)
	}
