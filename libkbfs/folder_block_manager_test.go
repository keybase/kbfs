...
func TestQuotaReclamationDeletedBlocks(t *testing.T) {
	...
	aNode1, _, err := kbfsOps1.CreateFile(ctx, rootNode1, "a", false, false)
	if err != nil {
		t.Fatalf("Couldn't create dir: %v", err)
	}
	...
	for _, name := range []string{"b", "c"} {
		node, _, err := kbfsOps1.CreateFile(ctx, rootNode1, name, false, false)
		if err != nil {
			t.Fatalf("Couldn't create dir: %v", err)
		}
	...
	dNode, _, err := kbfsOps2.CreateFile(ctx, rootNode2, "d", false, false)
	if err != nil {
	...
	eNode, _, err := kbfsOps2.CreateFile(ctx, rootNode2, "e", false, false)
	if err != nil {
		t.Fatalf("Couldn't create dir: %v", err)
	}
