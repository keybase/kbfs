...
// Fill in a successor copy of converted writer metadata.
func (wmd *WriterMetadataV2) fillSuccessorCopy(
	...
	*TLFWriterKeyBundleV3, error) {
	...
	for i, uid := range wmd.Writers {
		wmdCopy.Writers[i] = uid
	}
	wmdCopy.UnresolvedWriters = make([]keybase1.SocialAssertion, len(wmd.Extra.UnresolvedWriters))
	for i, uw := range wmd.Extra.UnresolvedWriters {
		wmdCopy.UnresolvedWriters[i] = uw
	}
...
func (md *BareRootMetadataV2) MakeSuccessorCopy(
	...
	if md.Version() < SegregatedKeyBundlesVer {
	...
	wkb, err := md.WriterMetadataV2.fillSuccessorCopy(
