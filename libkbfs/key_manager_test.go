...
func testKeyManagerRekeyAddAndRevokeDevice(t *testing.T, ver MetadataVer) {
	...
	_, err = RequestRekeyAndWaitForOneFinishEventForTest(kbfsOps1, rootNode1.GetFolderBranch().Tlf)
	if err != nil {
	...
	_, err = RequestRekeyAndWaitForOneFinishEventForTest(config2Dev3.KBFSOps(), rootNode1.GetFolderBranch().Tlf)
	if err != nil {
		t.Fatalf("Couldn't rekey: %+v", err)
	}
...
func testKeyManagerSelfRekeyAcrossDevices(t *testing.T, ver MetadataVer) {
	...
	kbfsOps2 := config2.KBFSOps()
	_, err = RequestRekeyAndWaitForOneFinishEventForTest(kbfsOps2, root2dev1.GetFolderBranch().Tlf)
	if err != nil {
		t.Fatalf("Couldn't rekey: %+v", err)
	}
...
func testKeyManagerReaderRekeyAndRevoke(t *testing.T, ver MetadataVer) {
	...
	root2Dev2 := GetRootNodeOrBust(ctx, t, config2Dev2, name, false)
	kbfsOps2Dev2 := config2Dev2.KBFSOps()
	_, err = RequestRekeyAndWaitForOneFinishEventForTest(kbfsOps2Dev2, root2Dev2.GetFolderBranch().Tlf)
	if err != nil {
...
func testKeyManagerRekeyBit(t *testing.T, ver MetadataVer) {
	...
	kbfsOps2Dev2 := config2Dev2.KBFSOps()
	_, err = RequestRekeyAndWaitForOneFinishEventForTest(kbfsOps2Dev2, rootNode1.GetFolderBranch().Tlf)
	if err != nil {
		t.Fatalf("Couldn't rekey: %+v", err)
	}
	...
	_, err = RequestRekeyAndWaitForOneFinishEventForTest(kbfsOps3Dev2, rootNode1.GetFolderBranch().Tlf)
	if err != nil {
		t.Fatalf("Couldn't rekey: %+v", err)
	}
