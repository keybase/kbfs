...
func TestKeyManagerRekeyAddAndRevokeDevice(t *testing.T) {
	...
	_, _, err = kbfsOps1.CreateFile(ctx, rootNode1, "d", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestKeyManagerSelfRekeyAcrossDevices(t *testing.T) {
	...
	_, _, err = kbfsOps2Dev2.CreateFile(ctx, root2dev2, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestKeyManagerRekeyAddDeviceWithPrompt(t *testing.T) {
	...
	_, _, err = kbfsOps2.CreateFile(ctx, rootNode2Dev2, "b", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
