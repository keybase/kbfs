...
func TestReporterSimpleMaxLimited(t *testing.T) {
	...
	err1 := errors.New("1")
	r.ReportErr(nil, "", false, IFCERFTReadMode, err1)
	checkReportedErrors(t, []error{err1}, r.AllKnownErrors())
	err2 := errors.New("2")
	r.ReportErr(nil, "", false, IFCERFTReadMode, err2)
	err3 := errors.New("3")
	r.ReportErr(nil, "", false, IFCERFTReadMode, err3)
	checkReportedErrors(t, []error{err1, err2, err3}, r.AllKnownErrors())
	err4 := errors.New("4")
	r.ReportErr(nil, "", false, IFCERFTReadMode, err4)
	checkReportedErrors(t, []error{err2, err3, err4}, r.AllKnownErrors())
	err5 := errors.New("5")
	r.ReportErr(nil, "", false, IFCERFTReadMode, err5)
	err6 := errors.New("6")
	r.ReportErr(nil, "", false, IFCERFTReadMode, err6)
