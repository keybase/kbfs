...
type folderBranchOps struct {
	...
	bid          IFCERFTBranchID // protected by mdWriterLock
	bType        branchType
	head         *IFCERFTRootMetadata
	...
	latestMergedRevision IFCERFTMetadataRevision
}
...
func newFolderBranchOps(config IFCERFTConfig, fb IFCERFTFolderBranch, bType branchType) *folderBranchOps {
	...
	fbo := &folderBranchOps{
		...
		blocks: folderBlockOps{
			...
			dirtyFiles: make(map[IFCERFTBlockPointer]*dirtyFile),
			unrefCache: make(map[IFCERFTBlockRef]*syncInfo),
			deCache:    make(map[IFCERFTBlockRef]IFCERFTDirEntry),
			deferredWrites: make(
				[]func(context.Context, *lockState, *IFCERFTRootMetadata, IFCERFTPath) error, 0),
			nodeCache: nodeCache,
		},
		nodeCache:       nodeCache,
		log:             log,
...
func (fbo *folderBranchOps) id() IFCERFTTlfID {
	return fbo.folderBranch.Tlf
}

func (fbo *folderBranchOps) branch() IFCERFTBranchName {
...
func (fbo *folderBranchOps) addToFavorites(ctx context.Context,
	...
	if head == nil {
		return IFCERFTOpsCantHandleFavorite{"Can't add a favorite without a handle"}
	}
...
func (fbo *folderBranchOps) deleteFromFavorites(ctx context.Context,
	...
	if head == nil {
		...
		return IFCERFTOpsCantHandleFavorite{"Can't delete a favorite without a handle"}
	}
...
func (fbo *folderBranchOps) getHead(lState *lockState) *IFCERFTRootMetadata {
...
func (fbo *folderBranchOps) isMasterBranch(lState *lockState) bool {
	...
	return fbo.bid == IFCERFTNullBranchID
}
...
func (fbo *folderBranchOps) setBranchIDLocked(lState *lockState, bid IFCERFTBranchID) {
	fbo.mdWriterLock.AssertLocked(lState)
...
func (fbo *folderBranchOps) checkDataVersion(p IFCERFTPath, ptr IFCERFTBlockPointer) error {
	if ptr.DataVer < IFCERFTFirstValidDataVer {
		return IFCERFTInvalidDataVersionError{ptr.DataVer}
	}
	// TODO: migrate back to fbo.config.DataVersion
	if ptr.DataVer > IFCERFTFilesWithHolesDataVer {
		return IFCERFTNewDataVersionError{p, ptr.DataVer}
	}
	return nil
}

func (fbo *folderBranchOps) setHeadLocked(
	ctx context.Context, lState *lockState, md *IFCERFTRootMetadata) error {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if isFirstHead && md.MergedStatus() == IFCERFTUnmerged {
		...
		fbo.cr.Resolve(md.Revision, IFCERFTMetadataRevisionUninitialized)
	} else if md.MergedStatus() == IFCERFTMerged {
...
func (fbo *folderBranchOps) setInitialHeadUntrustedLocked(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata) error {
	fbo.mdWriterLock.AssertLocked(lState)
	fbo.headLock.AssertLocked(lState)
	if fbo.head != nil {
...
func (fbo *folderBranchOps) setHeadSuccessorLocked(ctx context.Context,
	...
	if !resolvesTo {
		return IFCERFTIncompatibleHandleError{
			oldName,
...
func (fbo *folderBranchOps) identifyOnce(
	ctx context.Context, md *IFCERFTRootMetadata) error {
	fbo.identifyLock.Lock()
	defer fbo.identifyLock.Unlock()
	if fbo.identifyDone {
		return nil
...
func (fbo *folderBranchOps) getMDLocked(
	...
	md *IFCERFTRootMetadata, err error) {
	defer func() {
	...
	if rtype != mdWrite && rtype != mdRekey {
		return nil, IFCERFTMDWriteNeededInRequest{}
	}
	...
	md, err = mdops.GetUnmergedForTLF(ctx, fbo.id(), IFCERFTNullBranchID)
	if err != nil {
		return nil, err
	}
	...
	if md.data.Dir.Type != IFCERFTDir && (!md.IsInitialized() || md.IsReadable()) {
...
func (fbo *folderBranchOps) getMDForReadHelper(
	ctx context.Context, lState *lockState, rtype mdReqType) (*IFCERFTRootMetadata, error) {
	...
	if !md.ID.IsPublic() {
		...
		if !md.GetTlfHandle().IsReader(uid) {
			return nil, IFCERFTNewReadAccessError(md.GetTlfHandle(), username)
		}
	}
	return md, nil
}
...
func (fbo *folderBranchOps) getMDForReadNoIdentify(
	ctx context.Context, lState *lockState) (*IFCERFTRootMetadata, error) {
	return fbo.getMDForReadHelper(ctx, lState, mdReadNoIdentify)
}
...
func (fbo *folderBranchOps) getMDForWriteLocked(
	...
	if !md.GetTlfHandle().IsWriter(uid) {
		return nil,
			IFCERFTNewWriteAccessError(md.GetTlfHandle(), username)
	}
...
func (fbo *folderBranchOps) getMDForRekeyWriteLocked(
	ctx context.Context, lState *lockState) (rmd *IFCERFTRootMetadata, wasRekeySet bool, err error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if !handle.IsReader(uid) {
		return nil, false,
			IFCERFTNewRekeyPermissionError(md.GetTlfHandle(), username)
	}
	...
	if !handle.IsWriter(uid) && !newMd.IsWriterMetadataCopiedSet() {
		return nil, false,
			IFCERFTNewRekeyPermissionError(handle, username)
	}
...
func (fbo *folderBranchOps) initMDLocked(
	...
	if !handle.IsWriter(uid) {
		return IFCERFTNewWriteAccessError(handle, username)
	}

	newDblock := &IFCERFTDirBlock{
		Children: make(map[string]IFCERFTDirEntry),
	}

	var expectedKeyGen IFCERFTKeyGen
	var tlfCryptKey *IFCERFTTLFCryptKey
	if md.ID.IsPublic() {
		expectedKeyGen = IFCERFTPublicKeyGen
	} else {
		var rekeyDone bool
		...
		expectedKeyGen = IFCERFTFirstValidKeyGen
	}
	keyGen := md.LatestKeyGeneration()
	if keyGen != expectedKeyGen {
		return IFCERFTInvalidKeyGenerationError{handle, keyGen}
	}
	...
	now := fbo.nowUnixNano()
	md.data.Dir = IFCERFTDirEntry{
		IFCERFTBlockInfo: info,
	...
	md.AddOp(newCreateOp("", IFCERFTBlockPointer{}, IFCERFTDir))
	md.AddRefBlock(md.data.Dir.IFCERFTBlockInfo)
	md.UnrefBytes = 0

	if err = fbo.config.BlockOps().Put(ctx, md, info.IFCERFTBlockPointer,
		readyBlockData); err != nil {
		return err
	}
	if err = fbo.config.BlockCache().Put(
		info.IFCERFTBlockPointer, fbo.id(), newDblock, IFCERFTTransientEntry); err != nil {
		return err
	}
...
func (fbo *folderBranchOps) checkNode(node IFCERFTNode) error {
	...
	if fb != fbo.folderBranch {
		return IFCERFTWrongOpsError{fbo.folderBranch, fb}
	}
	return nil
}
...
func (fbo *folderBranchOps) CheckForNewMDAndInit(
	ctx context.Context, md *IFCERFTRootMetadata) (created bool, err error) {
	...
	err = runUnlessCanceled(ctx, func() error {
		fb := IFCERFTFolderBranch{md.ID, IFCERFTMasterBranch}
		if fb != fbo.folderBranch {
			return IFCERFTWrongOpsError{fbo.folderBranch, fb}
		}
		...
		if md.data.Dir.Type == IFCERFTDir {
...
func (fbo *folderBranchOps) execMDReadNoIdentifyThenMDWrite(
	...
	if _, ok := err.(IFCERFTMDWriteNeededInRequest); ok {
		fbo.mdWriterLock.Lock(lState)
		defer fbo.mdWriterLock.Unlock(lState)
...
func (fbo *folderBranchOps) getRootNode(ctx context.Context) (
	node IFCERFTNode, ei IFCERFTEntryInfo, handle *IFCERFTTlfHandle, err error) {
	...
	if err != nil {
		return nil, IFCERFTEntryInfo{}, nil, err
	}
	...
	handle = md.GetTlfHandle()
	node, err = fbo.nodeCache.GetOrCreate(md.data.Dir.IFCERFTBlockPointer,
		string(handle.GetCanonicalName()), nil)
	if err != nil {
		return nil, IFCERFTEntryInfo{}, nil, err
	}

	return node, md.Data().Dir.IFCERFTEntryInfo, handle, nil
}
...
// functions below.
func (fbo *folderBranchOps) pathFromNodeHelper(n IFCERFTNode) (IFCERFTPath, error) {
...
func (fbo *folderBranchOps) pathFromNodeForRead(n IFCERFTNode) (IFCERFTPath, error) {
...
func (fbo *folderBranchOps) GetDirChildren(ctx context.Context, dir IFCERFTNode) (
	children map[string]IFCERFTEntryInfo, err error) {
...
func (fbo *folderBranchOps) Lookup(ctx context.Context, dir IFCERFTNode, name string) (
	...
	err = runUnlessCanceled(ctx, func() error {
		...
		if de.Type == IFCERFTSym {
			...
			err = fbo.checkDataVersion(childPath, de.IFCERFTBlockPointer)
			if err != nil {
				return err
			}

			node, err = fbo.nodeCache.GetOrCreate(de.IFCERFTBlockPointer, name, dir)
			if err != nil {
				return err
			}
		}
	...
	return node, de.IFCERFTEntryInfo, nil
}
...
func (fbo *folderBranchOps) statEntry(ctx context.Context, node IFCERFTNode) (
	de IFCERFTDirEntry, err error) {
	err = fbo.checkNode(node)
	if err != nil {
		return IFCERFTDirEntry{}, err
	}
...
var zeroPtr IFCERFTBlockPointer
...
func (fbo *folderBranchOps) Stat(ctx context.Context, node IFCERFTNode) (
	...
	if err != nil {
		return IFCERFTEntryInfo{}, err
	}
	return de.IFCERFTEntryInfo, nil
}
...
// error).
func (bps *blockPutState) addNewBlock(blockPtr IFCERFTBlockPointer, block IFCERFTBlock, readyBlockData IFCERFTReadyBlockData, syncedCb func() error) {
...
func (fbo *folderBranchOps) readyBlockMultiple(ctx context.Context,
	md *IFCERFTRootMetadata, currBlock IFCERFTBlock, uid keybase1.UID, bps *blockPutState) (
	info IFCERFTBlockInfo, plainSize int, err error) {
	info, plainSize, readyBlockData, err :=
	...
	bps.addNewBlock(info.IFCERFTBlockPointer, currBlock, readyBlockData, nil)
	return
}

func (fbo *folderBranchOps) unembedBlockChanges(
	ctx context.Context, bps *blockPutState, md *IFCERFTRootMetadata, changes *AddBPSize, uid keybase1.UID) (err error) {
...
type localBcache map[IFCERFTBlockPointer]*IFCERFTDirBlock
...
func (fbo *folderBranchOps) syncBlock(
	...
	md *IFCERFTRootMetadata, newBlock IFCERFTBlock, dir IFCERFTPath, name string,
	entryType IFCERFTEntryType, mtime bool, ctime bool, stopAt IFCERFTBlockPointer, lbc localBcache) (IFCERFTPath, IFCERFTDirEntry, *blockPutState, error) {
	// now ready each dblock and write the DirEntry for the next one
	// in the path
	currBlock := newBlock
	currName := name
	newPath := IFCERFTPath{
		IFCERFTFolderBranch: dir.IFCERFTFolderBranch,
		path:                make([]IFCERFTPathNode, 0, len(dir.path)),
	}
	bps := newBlockPutState(len(dir.path))
	refPath := dir.ChildPathNoPtr(name)
	var newDe IFCERFTDirEntry
	doSetTime := true
	now := fbo.nowUnixNano()
	for len(newPath.path) < len(dir.path)+1 {
		...
		if err != nil {
			return IFCERFTPath{}, IFCERFTDirEntry{}, nil, err
		}

		// prepend to path and setup next one
		newPath.path = append([]IFCERFTPathNode{{info.IFCERFTBlockPointer, currName}},
			newPath.path...)

		// get the parent block
		prevIdx := len(dir.path) - len(newPath.path)
		var prevDblock *IFCERFTDirBlock
		var de IFCERFTDirEntry
		var nextName string
		nextDoSetTime := false
		if prevIdx < 0 {
			...
			if de, ok = prevDblock.Children[currName]; !ok {
				...
				if len(newPath.path) > 1 {
					return IFCERFTPath{}, IFCERFTDirEntry{}, nil, IFCERFTNoSuchNameError{currName}
				}
		...
		if prevIdx < 0 {
			md.AddUpdate(md.data.Dir.IFCERFTBlockInfo, info)
		} else if prevDe, ok := prevDblock.Children[currName]; ok {
			md.AddUpdate(prevDe.IFCERFTBlockInfo, info)
		} else {
			// this is a new block
			md.AddRefBlock(info)
		}
		...
		if prevIdx >= 0 && dir.path[prevIdx].IFCERFTBlockPointer == stopAt {
			// Put this back into the cache as dirty -- the next
			// syncBlock call will ready it.
			dblock, ok := currBlock.(*IFCERFTDirBlock)
			if !ok {
				return IFCERFTPath{}, IFCERFTDirEntry{}, nil, IFCERFTBadDataError{stopAt.ID}
			}
			lbc[stopAt] = dblock
			break
		}
...
func (fbo *folderBranchOps) syncBlockAndCheckEmbedLocked(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, newBlock IFCERFTBlock, dir IFCERFTPath, name string, entryType IFCERFTEntryType, mtime bool, ctime bool,
	stopAt IFCERFTBlockPointer, lbc localBcache) (
	IFCERFTPath, IFCERFTDirEntry, *blockPutState, error) {
	fbo.mdWriterLock.AssertLocked(lState)
...
func (fbo *folderBranchOps) doOneBlockPut(ctx context.Context,
	md *IFCERFTRootMetadata, blockState blockState,
	errChan chan error, blocksToRemoveChan chan *IFCERFTFileBlock) {
	...
	if err != nil {
		if isRecoverableBlockError(err) {
			fblock, ok := blockState.block.(*IFCERFTFileBlock)
			if ok && !fblock.IsInd {
...
func (fbo *folderBranchOps) doBlockPuts(ctx context.Context,
	md *IFCERFTRootMetadata, bps blockPutState) ([]IFCERFTBlockPointer, error) {
	...
	blocksToRemoveChan := make(chan *IFCERFTFileBlock, numWorkers)
	...
	err := <-errChan
	var blocksToRemove []IFCERFTBlockPointer
	if isRecoverableBlockError(err) {
...
func (fbo *folderBranchOps) finalizeMDWriteLocked(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, bps *blockPutState, excl IFCERFTEXCL) (err error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	mergedRev := IFCERFTMetadataRevisionUninitialized
	...
	if doUnmergedPut {
		...
		fbo.setBranchIDLocked(lState, IFCERFTNullBranchID)
...
func (fbo *folderBranchOps) finalizeMDRekeyWriteLocked(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata) (err error) {
	fbo.mdWriterLock.AssertLocked(lState)
...
func (fbo *folderBranchOps) finalizeGCOp(ctx context.Context, gco *gcOp) (
	...
	if md.MergedStatus() == IFCERFTUnmerged {
...
func (fbo *folderBranchOps) syncBlockAndFinalizeLocked(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, newBlock IFCERFTBlock, dir IFCERFTPath, name string, entryType IFCERFTEntryType, mtime bool, ctime bool,
	stopAt IFCERFTBlockPointer, excl IFCERFTEXCL) (de IFCERFTDirEntry, err error) {
	fbo.mdWriterLock.AssertLocked(lState)
...
func checkDisallowedPrefixes(name string) error {
	for _, prefix := range disallowedPrefixes {
		if strings.HasPrefix(name, prefix) {
			return IFCERFTDisallowedPrefixError{name, prefix}
		}
	}
	return nil
}
...
func (fbo *folderBranchOps) checkNewDirSize(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, dirPath IFCERFTPath, newName string) error {
	...
	if currSize+uint64(len(newName)) > fbo.config.MaxDirBytes() {
		return IFCERFTDirTooBigError{dirPath, currSize + uint64(len(newName)),
...
func (fbo *folderBranchOps) createEntryLocked(
	...
	entryType IFCERFTEntryType, excl IFCERFTEXCL) (IFCERFTNode, IFCERFTDirEntry, error) {
	fbo.mdWriterLock.AssertLocked(lState)

	if err := checkDisallowedPrefixes(name); err != nil {
		return nil, IFCERFTDirEntry{}, err
	}

	if uint32(len(name)) > fbo.config.MaxNameBytes() {
		return nil, IFCERFTDirEntry{},
			IFCERFTNameTooLongError{name, fbo.config.MaxNameBytes()}
	}
	...
	if _, ok := dblock.Children[name]; ok {
		return nil, IFCERFTDirEntry{}, IFCERFTNameExistsError{name}
	}
	...
	if entryType == IFCERFTDir {
		newBlock = &IFCERFTDirBlock{
			Children: make(map[string]IFCERFTDirEntry),
		}
	} else {
		newBlock = &IFCERFTFileBlock{}
	}
...
func (fbo *folderBranchOps) CreateDir(
	...
	n IFCERFTNode, ei IFCERFTEntryInfo, err error) {
	...
	err = fbo.doMDWriteWithRetryUnlessCanceled(ctx,
		func(lState *lockState) error {
			node, de, err := fbo.createEntryLocked(ctx, lState, dir, path, IFCERFTDir, IFCERFTNoEXCL)
			n = node
			ei = de.IFCERFTEntryInfo
			return err
		})
	if err != nil {
		return nil, IFCERFTEntryInfo{}, err
...
func (fbo *folderBranchOps) CreateFile(
	ctx context.Context, dir IFCERFTNode, path string, isExec bool, excl IFCERFTEXCL) (
	n IFCERFTNode, ei IFCERFTEntryInfo, err error) {
	...
	var entryType IFCERFTEntryType
	if isExec {
		entryType = IFCERFTExec
	} else {
		entryType = IFCERFTFile
	}
...
func (fbo *folderBranchOps) createLinkLocked(
	...
	toPath string) (IFCERFTDirEntry, error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if uint32(len(fromName)) > fbo.config.MaxNameBytes() {
		return IFCERFTDirEntry{},
			IFCERFTNameTooLongError{fromName, fbo.config.MaxNameBytes()}
	}
	...
	if _, ok := dblock.Children[fromName]; ok {
		return IFCERFTDirEntry{}, IFCERFTNameExistsError{fromName}
	}
	...
	md.AddOp(newCreateOp(fromName, dirPath.TailPointer(), IFCERFTSym))

	// Create a direntry for the link, and then sync
	now := fbo.nowUnixNano()
	dblock.Children[fromName] = IFCERFTDirEntry{
		IFCERFTEntryInfo: IFCERFTEntryInfo{
			Type:    IFCERFTSym,
	...
	_, err = fbo.syncBlockAndFinalizeLocked(
		...
		dirPath.TailName(), IFCERFTDir, true, true, zeroPtr, IFCERFTNoEXCL)
	if err != nil {
		return IFCERFTDirEntry{}, err
	}
...
func (fbo *folderBranchOps) unrefEntry(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, dir IFCERFTPath, de IFCERFTDirEntry, name string) error {
	md.AddUnrefBlock(de.IFCERFTBlockInfo)
	...
	// here.
	if de.Type == IFCERFTFile || de.Type == IFCERFTExec {
		blockInfos, err := fbo.blocks.GetIndirectFileBlockInfos(
			ctx, lState, md, childPath)
		if err != nil {
			return IFCERFTNoSuchBlockError{de.ID}
		}
...
func (fbo *folderBranchOps) removeEntryLocked(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, dir IFCERFTPath, name string) error {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if !ok {
		return IFCERFTNoSuchNameError{name}
	}
	...
	_, err = fbo.syncBlockAndFinalizeLocked(
		...
		IFCERFTDir, true, true, zeroPtr, IFCERFTNoEXCL)
	if err != nil {
		return err
	}
	return nil
...
func (fbo *folderBranchOps) removeDirLocked(ctx context.Context,
	...
	if !ok {
		return IFCERFTNoSuchNameError{dirName}
	}

	// construct a path for the child so we can check for an empty dir
	childPath := dirPath.ChildPath(dirName, de.IFCERFTBlockPointer)
	...
	if len(childBlock.Children) > 0 {
		return IFCERFTDirNotEmptyError{dirName}
	}
...
func (fbo *folderBranchOps) renameLocked(
	ctx context.Context, lState *lockState, oldParent IFCERFTPath, oldName string, newParent IFCERFTPath, newName string) (err error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if de, ok := newPBlock.Children[newName]; ok {
		...
		if de.Type == IFCERFTDir && newDe.Type != IFCERFTDir {
			return IFCERFTNotDirError{newParent.ChildPathNoPtr(newName)}
		} else if de.Type != IFCERFTDir && newDe.Type == IFCERFTDir {
			return IFCERFTNotFileError{newParent.ChildPathNoPtr(newName)}
		}

		if de.Type == IFCERFTDir {
			// The directory must be empty.
			oldTargetDir, err := fbo.blocks.GetDirBlockForReading(ctx, lState,
				md, de.IFCERFTBlockPointer, newParent.Branch,
			...
			if len(oldTargetDir.Children) != 0 {
				...
				return IFCERFTDirNotEmptyError{newName}
			}
		}
	...
	commonAncestor := oldParent.path[i].IFCERFTBlockPointer
	oldIsCommon := oldParent.TailPointer() == commonAncestor
	newIsCommon := newParent.TailPointer() == commonAncestor

	newOldPath := IFCERFTPath{IFCERFTFolderBranch: oldParent.IFCERFTFolderBranch}
	...
	if oldIsCommon {
		...
		newOldPath, _, oldBps, err = fbo.syncBlockAndCheckEmbedLocked(
			...
			IFCERFTDir, true, true, commonAncestor, lbc)
		if err != nil {
			return err
		}
	}
	...
	newNewPath, _, newBps, err := fbo.syncBlockAndCheckEmbedLocked(
		...
		IFCERFTDir, true, true, zeroPtr, lbc)
	if err != nil {
		return err
	}
	...
	newOldPath.path = append(make([]IFCERFTPathNode, i+1, i+1), newOldPath.path...)
	...
	return fbo.finalizeMDWriteLocked(ctx, lState, md, newBps, IFCERFTNoEXCL)
}
...
func (fbo *folderBranchOps) Rename(
	...
	return fbo.doMDWriteWithRetryUnlessCanceled(ctx,
		func(lState *lockState) error {
			...
			if oldParentPath.IFCERFTFolderBranch != newParentPath.IFCERFTFolderBranch {
...
func (fbo *folderBranchOps) setExLocked(
	ctx context.Context, lState *lockState, file IFCERFTPath, ex bool) (err error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if ex && (de.Type == IFCERFTFile) {
		de.Type = IFCERFTExec
	} else if !ex && (de.Type == IFCERFTExec) {
		de.Type = IFCERFTFile
	}
	...
	_, err = fbo.syncBlockAndFinalizeLocked(
		...
		IFCERFTDir, false, false, zeroPtr, IFCERFTNoEXCL)
	return err
}
...
func (fbo *folderBranchOps) setMtimeLocked(
	ctx context.Context, lState *lockState, file IFCERFTPath, mtime *time.Time) error {
	fbo.mdWriterLock.AssertLocked(lState)
...
func (fbo *folderBranchOps) syncLocked(ctx context.Context,
	lState *lockState, file IFCERFTPath) (stillDirty bool, err error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	if md.data.Dir.IFCERFTBlockPointer != file.path[0].IFCERFTBlockPointer {
	...
	newPath, _, newBps, err :=
		fbo.syncBlockAndCheckEmbedLocked(
			...
			file.TailName(), IFCERFTFile, true, true, zeroPtr, lbc)
	if err != nil {
		return true, err
	}
	...
	err = fbo.finalizeMDWriteLocked(ctx, lState, md, bps, IFCERFTNoEXCL)
	if err != nil {
		return true, err
	}
...
func (fbo *folderBranchOps) FolderStatus(
	...
	fbs IFCERFTFolderBranchStatus, updateChan <-chan IFCERFTStatusUpdate, err error) {
	fbo.log.CDebugf(ctx, "Status")
	defer func() { fbo.deferLog.CDebugf(ctx, "Done: %v", err) }()

	if folderBranch != fbo.folderBranch {
		return IFCERFTFolderBranchStatus{}, nil,
			IFCERFTWrongOpsError{fbo.folderBranch, folderBranch}
	}
...
func (fbo *folderBranchOps) searchForNode(ctx context.Context,
	ptr IFCERFTBlockPointer, md *IFCERFTRootMetadata) (IFCERFTNode, error) {
	// Record which pointers are new to this update, and thus worth
	// searching.
	newPtrs := make(map[IFCERFTBlockPointer]bool)
	...
	nodeMap, err := fbo.blocks.SearchForNodes(ctx, fbo.nodeCache, []IFCERFTBlockPointer{ptr},
		newPtrs, md)
	if err != nil {
		return nil, err
	}
	...
	n, ok := nodeMap[ptr]
	if !ok {
		return nil, IFCERFTNodeNotFoundError{ptr}
	}

	return n, nil
}

func (fbo *folderBranchOps) unlinkFromCache(op IFCERFTOps, oldDir IFCERFTBlockPointer, node IFCERFTNode, name string) error {
	...
	childPath.path[len(childPath.path)-2].IFCERFTBlockPointer = oldDir
	for _, ptr := range op.Unrefs() {
		childPath.path[len(childPath.path)-1].IFCERFTBlockPointer = ptr
...
func (fbo *folderBranchOps) updatePointers(op IFCERFTOps) {
	for _, update := range op.AllUpdates() {
...
func (fbo *folderBranchOps) notifyOneOpLocked(ctx context.Context,
	lState *lockState, op IFCERFTOps, md *IFCERFTRootMetadata) {
	fbo.headLock.AssertLocked(lState)
	...
	case *renameOp:
		...
		if oldNode != nil {
			...
			if newNode != nil {
				...
				var unrefPtr IFCERFTBlockPointer
				if oldNode != newNode {
...
func (fbo *folderBranchOps) getCurrMDRevisionLocked(lState *lockState) IFCERFTMetadataRevision {
...
func (fbo *folderBranchOps) getCurrMDRevision(
	lState *lockState) IFCERFTMetadataRevision {
	fbo.headLock.RLock(lState)
	defer fbo.headLock.RUnlock(lState)
	return fbo.getCurrMDRevisionLocked(lState)
}

type applyMDUpdatesFunc func(context.Context, *lockState, []*IFCERFTRootMetadata) error

func (fbo *folderBranchOps) applyMDUpdatesLocked(ctx context.Context,
	lState *lockState, rmds []*IFCERFTRootMetadata) error {
	fbo.mdWriterLock.AssertLocked(lState)
...
func (fbo *folderBranchOps) undoMDUpdatesLocked(ctx context.Context,
	...
	// require conflict resolution.
	if fbo.blocks.GetState(lState) != cleanState {
		return IFCERFTNotPermittedWhileDirtyError{}
	}
	...
	for i := len(rmds) - 1; i >= 0; i-- {
		...
		if rmd.Revision != fbo.getCurrMDRevisionLocked(lState) &&
			...
			return IFCERFTMDUpdateInvertError{rmd.Revision,
...
func (fbo *folderBranchOps) getLatestMergedRevision(lState *lockState) IFCERFTMetadataRevision {
	fbo.headLock.RLock(lState)
	defer fbo.headLock.RUnlock(lState)
...
func (fbo *folderBranchOps) setLatestMergedRevisionLocked(ctx context.Context, lState *lockState, rev IFCERFTMetadataRevision, allowBackward bool) {
	fbo.headLock.AssertLocked(lState)
...
func (fbo *folderBranchOps) getUnmergedMDUpdates(
	...
	IFCERFTMetadataRevision, []*IFCERFTRootMetadata, error) {
	// acquire mdWriterLock to read the current branch ID.
	bid := func() IFCERFTBranchID {
		fbo.mdWriterLock.Lock(lState)
		defer fbo.mdWriterLock.Unlock(lState)
		return fbo.bid
	}()
...
func (fbo *folderBranchOps) undoUnmergedMDUpdatesLocked(
	ctx context.Context, lState *lockState) ([]IFCERFTBlockPointer, error) {
	fbo.mdWriterLock.AssertLocked(lState)
	...
	rmds, err := getMDRange(ctx, fbo.config, fbo.id(), IFCERFTNullBranchID,
		currHead, currHead, IFCERFTMerged)
	if err != nil {
		return nil, err
	}
	if len(rmds) == 0 {
	...
	// Return all new refs
	var unmergedPtrs []IFCERFTBlockPointer
...
func (fbo *folderBranchOps) unstageLocked(ctx context.Context,
	...
	md.AddOp(resOp)
	return fbo.finalizeMDWriteLocked(ctx, lState, md, &blockPutState{}, IFCERFTNoEXCL)
}

// TODO: remove once we have automatic conflict resolution
func (fbo *folderBranchOps) UnstageForTesting(
	ctx context.Context, folderBranch IFCERFTFolderBranch) (err error) {
	fbo.log.CDebugf(ctx, "UnstageForTesting")
	defer func() { fbo.deferLog.CDebugf(ctx, "Done: %v", err) }()

	if folderBranch != fbo.folderBranch {
		return IFCERFTWrongOpsError{fbo.folderBranch, folderBranch}
	}
...
func (fbo *folderBranchOps) rekeyLocked(ctx context.Context,
	...
	if head != nil {
		...
		if err := fbo.getAndApplyMDUpdates(
			...
			if applyErr, ok := err.(IFCERFTMDRevisionMismatch); !ok ||
	...
	case nil:
		...
		md.Flags &= ^IFCERFTMetadataFlagRekey
		md.ClearLastRevision()

	case IFCERFTRekeyIncompleteError:
...
func (fbo *folderBranchOps) Rekey(ctx context.Context, tlf IFCERFTTlfID) (err error) {
	...
	err := fbo.runUnlessShutdown(func(ctx context.Context) error {
		...
		for {
			err := backoff.RetryNotifyWithContext(ctx, func() error {
				...
				if _, ok := err.(IFCERFTUnmergedError); ok {
...
func (fbo *folderBranchOps) finalizeResolution(ctx context.Context,
	lState *lockState, md *IFCERFTRootMetadata, bps *blockPutState,
...
func (fbo *folderBranchOps) GetUpdateHistory(ctx context.Context,
	...
	if folderBranch != fbo.folderBranch {
		return IFCERFTTLFUpdateHistory{}, IFCERFTWrongOpsError{fbo.folderBranch, folderBranch}
	}

	rmds, err := getMergedMDUpdates(ctx, fbo.config, fbo.id(),
		IFCERFTMetadataRevisionInitial)
	if err != nil {
		return IFCERFTTLFUpdateHistory{}, err
	}
	...
	history.Updates = make([]IFCERFTUpdateSummary, 0, len(rmds))
	...
	for _, rmd := range rmds {
		...
		updateSummary := IFCERFTUpdateSummary{
			Revision:  rmd.Revision,
			Date:      time.Unix(0, rmd.data.Dir.Mtime),
			Writer:    writer,
			LiveBytes: rmd.DiskUsage,
			Ops:       make([]IFCERFTOpSummary, 0, len(rmd.data.Changes.Ops)),
		}
		for _, op := range rmd.data.Changes.Ops {
			opSummary := IFCERFTOpSummary{
				Op:      op.String(),
