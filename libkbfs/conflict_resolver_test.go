...
func TestCRMergedChainsSimple(t *testing.T) {
	...
	// user1 makes a file
	_, _, err = config1.KBFSOps().CreateFile(ctx, dir1, "file1", false, false)
	if err != nil {
	...
	_, _, err = config2.KBFSOps().CreateFile(ctx, dir2, "file2", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRMergedChainsDifferentDirectories(t *testing.T) {
	...
	_, _, err = config1.KBFSOps().CreateFile(ctx, dirA1, "file1", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	...
	_, _, err = config2.KBFSOps().CreateFile(ctx, dirB2, "file2", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRMergedChainsDeletedDirectories(t *testing.T) {
	...
	_, _, err = config2.KBFSOps().CreateFile(ctx, dirC2, "file2", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRMergedChainsComplex(t *testing.T) {
	...
	_, _, err = config1.KBFSOps().CreateFile(ctx, dirD1, "file5", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	...
	_, _, err = config2.KBFSOps().CreateFile(ctx, dirJ2, "file2", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	_, _, err = config2.KBFSOps().CreateFile(ctx, dirF2, "file3", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	_, _, err = config2.KBFSOps().CreateFile(ctx, dirC2, "file4", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
...
func TestCRMergedChainsConflictSimple(t *testing.T) {
	...
	// user1 creates file1
	_, _, err = config1.KBFSOps().CreateFile(ctx, dirRoot1, "file1", false, false)
	if err != nil {
		t.Fatalf("Couldn't make file: %v", err)
	}

	// user2 also create file1, but makes it executable
	_, _, err = config2.KBFSOps().CreateFile(ctx, dirRoot2, "file1", true, false)
	if err != nil {
		t.Fatalf("Couldn't make dir: %v", err)
	}
...
func TestCRMergedChainsConflictFileCollapse(t *testing.T) {
	...
	// user1 creates file
	_, _, err = config1.KBFSOps().CreateFile(ctx, dirRoot1, "file", false, false)
	if err != nil {
		t.Fatalf("Couldn't make file: %v", err)
	}
...
func TestCRDoActionsWriteConflict(t *testing.T) {
	...
	// user1 makes a file
	file1, _, err := config1.KBFSOps().CreateFile(ctx, dir1, "file", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
