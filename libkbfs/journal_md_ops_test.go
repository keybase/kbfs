...
func makeMDForJournalMDOpsTest(
	...
	revision tlf.MetadataRevision) *RootMetadata {
...
func TestJournalMDOpsBasics(t *testing.T) {
	...
	rmd := makeMDForJournalMDOpsTest(t, config, id, h, tlf.MetadataRevision(1))
	...
	for i := tlf.MetadataRevision(2); i < 8; i++ {
		rmd.SetRevision(tlf.MetadataRevision(i))
	...
	require.Equal(t, tlf.MetadataRevision(7), head.Revision())
	...
	rmd.SetRevision(tlf.MetadataRevision(8))
	rmd.SetPrevRoot(prevRoot)
	...
	for i := tlf.MetadataRevision(8); i <= 10; i++ {
		rmd.SetRevision(tlf.MetadataRevision(i))
		rmd.SetPrevRoot(prevRoot)
		mdID, err := mdOps.Put(ctx, rmd)
		require.NoError(t, err, "i=%d", i)
	...
	require.NoError(t, err)
	require.NotEqual(t, ImmutableRootMetadata{}, head)
	require.Equal(t, tlf.MetadataRevision(8), head.Revision())
	...
	require.NoError(t, err)
	require.NotEqual(t, ImmutableRootMetadata{}, head)
	require.Equal(t, tlf.MetadataRevision(10), head.Revision())
	...
	//     middle merged block.
	for i := tlf.MetadataRevision(11); i < 41; i++ {
		rmd.SetRevision(tlf.MetadataRevision(i))
		rmd.SetPrevRoot(prevRoot)
	...
	for i := tlf.MetadataRevision(8); i <= 14; i++ {
		require.Equal(t, i, rmdses[i-8].Revision())
	}
	...
	for i := tlf.MetadataRevision(1); i <= 8; i++ {
...
func TestJournalMDOpsPutUnmerged(t *testing.T) {
	...
	rmd := makeMDForJournalMDOpsTest(t, config, id, h, tlf.MetadataRevision(2))
...
func TestJournalMDOpsLocalSquashBranch(t *testing.T) {
	...
	firstRevision := tlf.MetadataRevision(1)
	...
	stopRevision := firstRevision + tlf.MetadataRevision(mdCount*2)
	...
	require.Equal(t, firstRevision+tlf.MetadataRevision(1), irmds[0].Revision())
}
