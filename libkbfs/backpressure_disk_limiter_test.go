...
func TestBackpressureDiskLimiterCounters(t *testing.T) {
	...
	availBytes := bdl.onJournalEnable(ctx, 10, 0)
	...
	availBytes = bdl.onJournalEnable(ctx, 440, 0)
	...
	availBytes = bdl.onJournalEnable(ctx, 0, 0)
	...
	availBytes, err = bdl.beforeBlockPut(context.Background(), 10, 0)
	require.NoError(t, err)
	...
	availBytes, err = bdl.beforeBlockPut(context.Background(), 9, 0)
	require.NoError(t, err)
...
func TestBackpressureDiskLimiterLargeDiskDelay(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		_, err = bdl.beforeBlockPut(ctx, blockSize, 0)
		require.NoError(t, err)
	...
	cancel2()
	_, err = bdl.beforeBlockPut(ctx2, blockSize, 0)
	...
	journalBytes, freeBytes, bytesSemaphoreMax = bdl.getLockedVarsForTest()
	require.Equal(t, int64(bytesPut), journalBytes)
	require.Equal(t, int64(math.MaxInt64), freeBytes)
	require.Equal(t, int64(100), bytesSemaphoreMax)
	require.Equal(t, int64(100-bytesPut), bdl.bytesSemaphore.Count())
}
