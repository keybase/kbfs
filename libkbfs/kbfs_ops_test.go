...
func (cbo *CheckBlockOps) Get(ctx context.Context, md *IFCERFTRootMetadata, blockPtr IFCERFTBlockPointer, block IFCERFTBlock) error {
...
func (cbo *CheckBlockOps) Put(ctx context.Context, md *IFCERFTRootMetadata, blockPtr IFCERFTBlockPointer, readyBlockData IFCERFTReadyBlockData) error {
...
func (cbo *CheckBlockOps) Archive(ctx context.Context, md *IFCERFTRootMetadata, ptrs []IFCERFTBlockPointer) error {
...
func TestKBFSOpsGetFavoritesSuccess(t *testing.T) {
	...
	// dup for testing
	handles := []*IFCERFTTlfHandle{handle1, handle2, handle2}
...
func getOps(config IFCERFTConfig, id IFCERFTTlfID) *folderBranchOps {
	return config.KBFSOps().(*KBFSOpsStandard).
		getOpsNoAdd(IFCERFTFolderBranch{id, IFCERFTMasterBranch})
}
...
func createNewRMD(t *testing.T, config IFCERFTConfig, name string, public bool) (
	IFCERFTTlfID, *IFCERFTTlfHandle, *IFCERFTRootMetadata) {
...
func injectNewRMD(t *testing.T, config *ConfigMock) (
	keybase1.UID, IFCERFTTlfID, *IFCERFTRootMetadata) {
	id, h, rmd := createNewRMD(t, config, "alice", false)
	var keyGen IFCERFTKeyGen
	if id.IsPublic() {
		keyGen = IFCERFTPublicKeyGen
	} else {
		keyGen = 1
	}
	...
	config.Notifier().RegisterForChanges(
		[]IFCERFTFolderBranch{{id, IFCERFTMasterBranch}}, config.observer)
...
func TestKBFSOpsGetRootNodeCacheSuccess(t *testing.T) {
	...
	rmd.data.Dir.IFCERFTBlockPointer.ID = fakeBlockID(1)
	...
	assert.Equal(t, rmd.data.Dir.IFCERFTEntryInfo, ei)
...
func (kbpki failIdentifyKBPKI) Identify(ctx context.Context, assertion, reason string) (IFCERFTUserInfo, error) {
	return IFCERFTUserInfo{}, kbpki.identifyErr
}
...
func expectBlock(config *ConfigMock, rmd *IFCERFTRootMetadata, blockPtr IFCERFTBlockPointer, block IFCERFTBlock, err error) {
...
func fillInNewMD(t *testing.T, config *ConfigMock, rmd *IFCERFTRootMetadata) (
	rootPtr IFCERFTBlockPointer, plainSize int, readyBlockData IFCERFTReadyBlockData) {
	if !rmd.ID.IsPublic() {
		config.mockKeyman.EXPECT().Rekey(gomock.Any(), rmd, gomock.Any()).
			Do(func(ctx context.Context, rmd *IFCERFTRootMetadata, promptPaper bool) {
				FakeInitialRekey(rmd, rmd.GetTlfHandle().ToBareHandleOrBust())
			}).Return(true, nil, nil)
	}
	rootPtr = IFCERFTBlockPointer{
		ID:      fakeBlockID(42),
		KeyGen:  1,
		DataVer: 1,
	}
...
func TestKBFSOpsGetRootMDCreateNewFailNonWriter(t *testing.T) {
	...
	expectedErr := IFCERFTWriteAccessError{
...
func TestKBFSOpsGetRootMDForHandleExisting(t *testing.T) {
	...
	ops.head = rmd
	n, ei, err :=
		config.KBFSOps().GetOrCreateRootNode(ctx, h, IFCERFTMasterBranch)
	require.NoError(t, err)
	assert.True(t, fboIdentityDone(ops))
...
func nodeFromPath(t *testing.T, ops *folderBranchOps, p IFCERFTPath) IFCERFTNode {
	var prevNode IFCERFTNode
	// populate the node cache with all the nodes we'll need
	for _, pathNode := range p.path {
		n, err := ops.nodeCache.GetOrCreate(pathNode.IFCERFTBlockPointer,
...
func testPutBlockInCache(config *ConfigMock, ptr IFCERFTBlockPointer, id IFCERFTTlfID, block IFCERFTBlock) {
...
func TestKBFSOpsGetBaseDirChildrenCacheSuccess(t *testing.T) {
	...
	for c, ei := range children {
		...
		} else if de.IFCERFTEntryInfo != ei {
...
func TestKBFSOpsGetBaseDirChildrenUncachedFailNonReader(t *testing.T) {
	...
	ops.head = rmd
	expectedErr := IFCERFTReadAccessError{"alice", h.GetCanonicalName(), false}
	if _, err := config.KBFSOps().GetDirChildren(ctx, n); err == nil {
...
func TestKBFSOpsGetBaseDirChildrenUncachedFailMissingBlock(t *testing.T) {
	...
	// fail block fetch
	err := IFCERFTNoSuchBlockError{rootID}
...
func TestKBFSOpsGetNestedDirChildrenCacheSuccess(t *testing.T) {
	...
	p := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode, bNode}}
	n := nodeFromPath(t, ops, p)

	testPutBlockInCache(config, bNode.IFCERFTBlockPointer, id, dirBlock)
...
func TestKBFSOpsLookupSuccess(t *testing.T) {
	...
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, dirBlock)
	...
	expectedBNode := IFCERFTPathNode{makeBP(bID, rmd, config, u), "b"}
	expectedBNode.KeyGen = 1
	if ei != dirBlock.Children["b"].IFCERFTEntryInfo {
		t.Errorf("Lookup returned a bad entry info: %v vs %v",
			ei, dirBlock.Children["b"].IFCERFTEntryInfo)
...
func TestKBFSOpsLookupNoSuchNameFail(t *testing.T) {
	...
	expectedErr := IFCERFTNoSuchNameError{"c"}
...
func TestKBFSOpsLookupNewDataVersionFail(t *testing.T) {
	...
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, dirBlock)
	expectedErr := &IFCERFTNewDataVersionError{
		IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode, bNode}},
		bInfo.DataVer,
	}
...
func expectSyncBlockHelper(
	...
	// construct new path
	newPath := IFCERFTPath{
		IFCERFTFolderBranch{Tlf: id},
		make([]IFCERFTPathNode, 0, len(p.path)+1),
	}
	for _, node := range p.path {
		newPath.path = append(newPath.path, IFCERFTPathNode{Name: node.Name})
	}
	if newEntry {
		// one for the new entry
		newPath.path = append(newPath.path, IFCERFTPathNode{Name: name})
	}
	...
	for i := len(newPath.path) - 1; i >= skipSync; i-- {
		...
		config.mockBops.EXPECT().Put(gomock.Any(), rmdMatcher{rmd},
			ptrMatcher{newPath.path[i].IFCERFTBlockPointer}, readyBlockData).
			Return(nil)
	}
	if skipSync == 0 {
		...
		if isUnmerged {
			...
			if rmd.BID == IFCERFTNullBranchID {
				config.mockCrypto.EXPECT().MakeRandomBranchID().Return(IFCERFTBranchID{}, nil)
			}
			config.mockMdops.EXPECT().PutUnmerged(
				gomock.Any(), gomock.Any(), gomock.Any()).
				Do(func(ctx context.Context, rmd *IFCERFTRootMetadata, bid IFCERFTBranchID) {
					// add some serialized metadata to satisfy the check
					rmd.SerializedPrivateMetadata = make([]byte, 1)
				}).Return(nil)
		} else {
			config.mockMdops.EXPECT().Put(gomock.Any(), gomock.Any()).
				Do(func(ctx context.Context, rmd *IFCERFTRootMetadata) {
					// add some serialized metadata to satisfy the check
					rmd.SerializedPrivateMetadata = make([]byte, 1)
				}).Return(nil)
		}
		config.mockMdcache.EXPECT().Put(gomock.Any()).
			Do(func(rmd *IFCERFTRootMetadata) {
				*newRmd = rmd
...
func getBlockFromCache(t *testing.T, config IFCERFTConfig, ptr IFCERFTBlockPointer, branch IFCERFTBranchName) IFCERFTBlock {
...
func checkNewPath(t *testing.T, ctx context.Context, config IFCERFTConfig, newPath IFCERFTPath, expectedPath IFCERFTPath, rmd *IFCERFTRootMetadata, blocks []BlockID,
	...
	for i, id := range blocks {
		...
		if (currDe.Type != IFCERFTFile && currDe.Type != IFCERFTExec) && currDe.Size == 0 {
			t.Errorf("Type %s unexpectedly has 0 size (%d)", currDe.Type, i)
		}
	}
}

func checkBPs(t *testing.T, bps []IFCERFTBlockPointer, expectedBPs []IFCERFTBlockPointer, kind string) {
...
func checkOp(t *testing.T, op OpCommon, refs []IFCERFTBlockPointer, unrefs []IFCERFTBlockPointer, updates []IFCERFTBlockUpdate) {
...
func testCreateEntrySuccess(t *testing.T, entryType IFCERFTEntryType) {
	mockCtrl, config, ctx := kbfsOpsInit(t, true)
	defer kbfsTestShutdown(mockCtrl, config)
	...
	// creating "a/b"
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, aBlock)
	...
	switch entryType {
	case IFCERFTFile:
		newN, _, err = config.KBFSOps().CreateFile(ctx, n, "b", false, IFCERFTNoEXCL)
	case IFCERFTExec:
		newN, _, err = config.KBFSOps().CreateFile(ctx, n, "b", true, IFCERFTNoEXCL)
	case IFCERFTDir:
	...
	checkNewPath(t, ctx, config, newP, expectedPath, newRmd, blocks,
		entryType, "b", false)
	b1 :=
		getDirBlockFromCache(t, config, newP.path[1].IFCERFTBlockPointer, newP.Branch)
	...
	if entryType != IFCERFTSym {
		refBlocks = append(refBlocks, newP.path[2].IFCERFTBlockPointer)
	}
	updates := []IFCERFTBlockUpdate{
		{rmd.data.Dir.IFCERFTBlockPointer, newP.path[0].IFCERFTBlockPointer},
	}
	checkOp(t, co.OpCommon, refBlocks, nil, updates)
	dirUpdate := IFCERFTBlockUpdate{rootBlock.Children["a"].IFCERFTBlockPointer,
		newP.path[1].IFCERFTBlockPointer}
	if co.Dir != dirUpdate {
...
func TestKBFSOpsCreateDirSuccess(t *testing.T) {
	testCreateEntrySuccess(t, IFCERFTDir)
}

func TestKBFSOpsCreateFileSuccess(t *testing.T) {
	testCreateEntrySuccess(t, IFCERFTFile)
}

func TestKBFSOpsCreateExecFileSuccess(t *testing.T) {
	testCreateEntrySuccess(t, IFCERFTExec)
}

func TestKBFSOpsCreateLinkSuccess(t *testing.T) {
	testCreateEntrySuccess(t, IFCERFTSym)
}
...
func testCreateEntryFailDupName(t *testing.T, isDir bool) {
	...
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	expectedErr := IFCERFTNameExistsError{"a"}
...
func testCreateEntryFailNameTooLong(t *testing.T, isDir bool) {
	...
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	expectedErr := IFCERFTNameTooLongError{name, config.maxNameBytes}
...
func testCreateEntryFailDirTooBig(t *testing.T, isDir bool) {
	...
	if err == nil {
		t.Errorf("Got no expected error on create")
	} else if _, ok := err.(IFCERFTDirTooBigError); !ok {
		t.Errorf("Got unexpected error on create: %v", err)
	}
}
...
func testCreateEntryFailKBFSPrefix(t *testing.T, et IFCERFTEntryType) {
	mockCtrl, config, ctx := kbfsOpsInit(t, false)
	defer kbfsTestShutdown(mockCtrl, config)
	...
	name := ".kbfs_status"
	expectedErr := IFCERFTDisallowedPrefixError{name, ".kbfs"}
	...
	case IFCERFTExec:
		_, _, err = config.KBFSOps().CreateFile(ctx, n, name, true, IFCERFTNoEXCL)
	case IFCERFTFile:
		_, _, err = config.KBFSOps().CreateFile(ctx, n, name, false, IFCERFTNoEXCL)
	}
	if err == nil {
		t.Errorf("Got no expected error on create")
	} else if err != expectedErr {
...
func TestCreateDirFailKBFSPrefix(t *testing.T) {
	testCreateEntryFailKBFSPrefix(t, IFCERFTDir)
}

func TestCreateFileFailKBFSPrefix(t *testing.T) {
	testCreateEntryFailKBFSPrefix(t, IFCERFTFile)
}

func TestCreateExecFailKBFSPrefix(t *testing.T) {
	testCreateEntryFailKBFSPrefix(t, IFCERFTExec)
}

func TestCreateLinkFailKBFSPrefix(t *testing.T) {
	testCreateEntryFailKBFSPrefix(t, IFCERFTSym)
}

func testRemoveEntrySuccess(t *testing.T, entryType IFCERFTEntryType) {
	mockCtrl, config, ctx := kbfsOpsInit(t, true)
	defer kbfsTestShutdown(mockCtrl, config)
	...
	node := IFCERFTPathNode{makeBP(rootID, rmd, config, uid), "p"}
	aNode := IFCERFTPathNode{makeBP(aID, rmd, config, uid), "a"}
	bNode := IFCERFTPathNode{makeBP(bID, rmd, config, uid), "b"}
	p := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode}}
	ops := getOps(config, id)
	n := nodeFromPath(t, ops, p)
	...
	// deleting "a/b"
	if entryType != IFCERFTSym {
		testPutBlockInCache(config, bNode.IFCERFTBlockPointer, id, bBlock)
	}
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, aBlock)
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	// sync block
	...
	unrefBlocks := []IFCERFTBlockPointer{bNode.IFCERFTBlockPointer}
	updates := []IFCERFTBlockUpdate{
		{rmd.data.Dir.IFCERFTBlockPointer, newP.path[0].IFCERFTBlockPointer},
	}
...
func TestKBFSOpsRemoveDirSuccess(t *testing.T) {
	testRemoveEntrySuccess(t, IFCERFTDir)
}

func TestKBFSOpsRemoveFileSuccess(t *testing.T) {
	testRemoveEntrySuccess(t, IFCERFTFile)
}

func TestKBFSOpsRemoveSymlinkSuccess(t *testing.T) {
	testRemoveEntrySuccess(t, IFCERFTSym)
}
...
func TestKBFSOpRemoveMultiBlockFileSuccess(t *testing.T) {
	...
	expectBlock(config, rmd, fileNode.IFCERFTBlockPointer, fileBlock, nil)

	testPutBlockInCache(config, fileBlock.IPtrs[0].IFCERFTBlockPointer, id, block1)
	testPutBlockInCache(config, fileBlock.IPtrs[1].IFCERFTBlockPointer, id, block2)
	testPutBlockInCache(config, fileBlock.IPtrs[2].IFCERFTBlockPointer, id, block3)
	testPutBlockInCache(config, fileBlock.IPtrs[3].IFCERFTBlockPointer, id, block4)
	...
	checkNewPath(t, ctx, config, newP, expectedPath, newRmd, blocks,
		IFCERFTFile, "", false)
	b0 :=
		getDirBlockFromCache(t, config, newP.path[0].IFCERFTBlockPointer, newP.Branch)
	...
	unrefBlocks := []IFCERFTBlockPointer{
		{ID: fileID, KeyGen: 1},
		fileBlock.IPtrs[0].IFCERFTBlockPointer,
		fileBlock.IPtrs[1].IFCERFTBlockPointer,
		fileBlock.IPtrs[2].IFCERFTBlockPointer,
		fileBlock.IPtrs[3].IFCERFTBlockPointer,
	}
	checkOp(t, ro.OpCommon, nil, unrefBlocks, nil)
	dirUpdate := IFCERFTBlockUpdate{rmd.data.Dir.IFCERFTBlockPointer,
		newP.path[0].IFCERFTBlockPointer}
	if ro.Dir != dirUpdate {
		t.Errorf("Incorrect dir update in op: %v vs. %v", ro.Dir, dirUpdate)
...
func TestRemoveDirFailNonEmpty(t *testing.T) {
	...
	testPutBlockInCache(config, bNode.IFCERFTBlockPointer, id, bBlock)
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, aBlock)
	expectedErr := IFCERFTDirNotEmptyError{bNode.Name}
...
func TestRemoveDirFailNoSuchName(t *testing.T) {
	...
	testPutBlockInCache(config, bNode.IFCERFTBlockPointer, id, bBlock)
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, aBlock)
	expectedErr := IFCERFTNoSuchNameError{bNode.Name}
...
func TestRenameInDirSuccess(t *testing.T) {
	...
	b1 := getDirBlockFromCache(
		t, config, newP.path[1].IFCERFTBlockPointer, newP.Branch)
	if _, ok := b1.Children["b"]; ok {
	...
	oldDirUpdate := IFCERFTBlockUpdate{aNode.IFCERFTBlockPointer, newP.path[1].IFCERFTBlockPointer}
	newDirUpdate := IFCERFTBlockUpdate{}
...
func TestRenameInDirOverEntrySuccess(t *testing.T) {
	...
	checkOp(t, ro.OpCommon, nil, []IFCERFTBlockPointer{cNode.IFCERFTBlockPointer}, updates)
	oldDirUpdate := IFCERFTBlockUpdate{aNode.IFCERFTBlockPointer, newP.path[1].IFCERFTBlockPointer}
	newDirUpdate := IFCERFTBlockUpdate{}
	if ro.OldDir != oldDirUpdate {
		t.Errorf("Incorrect old dir update in op: %v vs. %v", ro.OldDir,
...
func TestRenameInRootSuccess(t *testing.T) {
	...
	b0 := getDirBlockFromCache(
		t, config, newP.path[0].IFCERFTBlockPointer, newP.Branch)
	if _, ok := b0.Children["a"]; ok {
	...
	oldDirUpdate := IFCERFTBlockUpdate{rmd.data.Dir.IFCERFTBlockPointer,
		newP.path[0].IFCERFTBlockPointer}
	newDirUpdate := IFCERFTBlockUpdate{}
	if ro.OldDir != oldDirUpdate {
		t.Errorf("Incorrect old dir update in op: %v vs. %v", ro.OldDir,
...
func TestRenameAcrossDirsSuccess(t *testing.T) {
	...
	node := IFCERFTPathNode{makeBP(rootID, rmd, config, uid), "p"}
	aNode := IFCERFTPathNode{makeBP(aID, rmd, config, uid), "a"}
	p1 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode}}
	ops := getOps(config, id)
	...
	// renaming "a/b" to "d/c"
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, aBlock)
	testPutBlockInCache(config, dNode.IFCERFTBlockPointer, id, dBlock)
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	...
	checkNewPath(t, ctx, config, newP2, expectedPath2, newRmd, blocks2,
		IFCERFTFile, "c", true)
	b0 := getDirBlockFromCache(
		t, config, newP1.path[0].IFCERFTBlockPointer, newP1.Branch)
	...
	updates := []IFCERFTBlockUpdate{
		{rmd.data.Dir.IFCERFTBlockPointer, newP1.path[0].IFCERFTBlockPointer},
	}
	checkOp(t, ro.OpCommon, nil, nil, updates)
	oldDirUpdate := IFCERFTBlockUpdate{aNode.IFCERFTBlockPointer, newP1.path[1].IFCERFTBlockPointer}
	newDirUpdate := IFCERFTBlockUpdate{dNode.IFCERFTBlockPointer, newP2.path[1].IFCERFTBlockPointer}
	if ro.OldDir != oldDirUpdate {
		t.Errorf("Incorrect old dir update in op: %v vs. %v", ro.OldDir,
			oldDirUpdate)
	} else if ro.OldName != "b" {
...
func TestRenameAcrossPrefixSuccess(t *testing.T) {
	...
	dNode := IFCERFTPathNode{makeBP(dID, rmd, config, uid), "d"}
	p1 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode}}
	p2 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode, dNode}}
	ops := getOps(config, id)
	n1 := nodeFromPath(t, ops, p1)
	n2 := nodeFromPath(t, ops, p2)
	...
	checkNewPath(t, ctx, config, newP2, expectedPath2, newRmd, blocks,
		IFCERFTFile, "c", true)
	b1 := getDirBlockFromCache(
		t, config, newP1.path[1].IFCERFTBlockPointer, newP1.Branch)
	if _, ok := b1.Children["b"]; ok {
		t.Errorf("entry for b is still around after rename")
	} else if len(config.observer.batchChanges) != 2 {
		t.Errorf("Expected 2 batch notifications, got %d",
...
func TestRenameAcrossOtherPrefixSuccess(t *testing.T) {
	...
	aNode := IFCERFTPathNode{makeBP(aID, rmd, config, uid), "a"}
	dNode := IFCERFTPathNode{makeBP(dID, rmd, config, uid), "d"}
	p1 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode, dNode}}
	p2 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, aNode}}
	ops := getOps(config, id)
	n1 := nodeFromPath(t, ops, p1)
	n2 := nodeFromPath(t, ops, p2)
	...
	b2 := getDirBlockFromCache(
		t, config, newP1.path[2].IFCERFTBlockPointer, newP1.Branch)
...
func TestRenameFailAcrossTopLevelFolders(t *testing.T) {
	...
	aID1 := fakeBlockID(42)
	node1 := IFCERFTPathNode{makeBP(rootID1, rmd1, config, uid1), "p"}
	aNode1 := IFCERFTPathNode{makeBP(aID1, rmd1, config, uid1), "a"}
	p1 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id1}, []IFCERFTPathNode{node1, aNode1}}
	ops1 := getOps(config, id1)
	n1 := nodeFromPath(t, ops1, p1)

	rootID2 := fakeBlockID(38)
	aID2 := fakeBlockID(39)
	node2 := IFCERFTPathNode{makeBP(rootID2, rmd2, config, uid2), "p"}
	aNode2 := IFCERFTPathNode{makeBP(aID2, rmd2, config, uid2), "a"}
	p2 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id2}, []IFCERFTPathNode{node2, aNode2}}
	ops2 := getOps(config, id2)
	n2 := nodeFromPath(t, ops2, p2)

	expectedErr := IFCERFTRenameAcrossDirsError{}
...
func TestRenameFailAcrossBranches(t *testing.T) {
	...
	aNode1 := IFCERFTPathNode{makeBP(aID1, rmd1, config, uid1), "a"}
	p1 := IFCERFTPath{IFCERFTFolderBranch{Tlf: id1}, []IFCERFTPathNode{node1, aNode1}}
	p2 := IFCERFTPath{IFCERFTFolderBranch{id1, "test"}, []IFCERFTPathNode{node1, aNode1}}
	ops1 := getOps(config, id1)
	n1 := nodeFromPath(t, ops1, p1)
	ops2 := config.KBFSOps().(*KBFSOpsStandard).getOpsNoAdd(
		IFCERFTFolderBranch{id1, "test"})
	n2 := nodeFromPath(t, ops2, p2)
...
func TestKBFSOpsServerReadFullSuccess(t *testing.T) {
	...
	fileNode := IFCERFTPathNode{fileBlockPtr, "f"}
	p := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, fileNode}}
	ops := getOps(config, id)
	pNode := nodeFromPath(t, ops, p)
...
func TestKBFSOpsWriteNewBlockSuccess(t *testing.T) {
	...
	newFileBlock := getFileBlockFromCache(t, config, fileNode.IFCERFTBlockPointer,
		p.Branch)
	newRootBlock := getDirBlockFromCache(t, config, node.IFCERFTBlockPointer, p.Branch)
	lState := makeFBOLockState()
...
func mergeUnrefCache(
	ops *folderBranchOps, lState *lockState, file IFCERFTPath, md *IFCERFTRootMetadata) {
	ops.blocks.blockLock.RLock(lState)
	defer ops.blocks.blockLock.RUnlock(lState)
...
func TestKBFSOpsWriteFailTooBig(t *testing.T) {
	...
	} else if _, ok := err.(IFCERFTFileTooBigError); !ok {
...
func TestKBFSOpsTruncateShortensLastBlock(t *testing.T) {
	...
	ops := getOps(config, id)
	n := nodeFromPath(t, ops, p)
	rmd.AddOp(newSyncOp(fileInfo.IFCERFTBlockPointer))
	...
	newPBlock := getFileBlockFromCache(t, config, fileNode.IFCERFTBlockPointer,
		p.Branch)
	newBlock1 := getFileBlockFromCache(t, config,
		fileBlock.IPtrs[0].IFCERFTBlockPointer, p.Branch)
	newBlock2 := getFileBlockFromCache(t, config,
...
func testSetExSuccess(t *testing.T, entryType IFCERFTEntryType, ex bool) {
	mockCtrl, config, ctx := kbfsOpsInit(t, entryType != IFCERFTSym)
	defer kbfsTestShutdown(mockCtrl, config)
	...
	var expectedType IFCERFTEntryType
	if entryType == IFCERFTFile && ex {
		expectedType = IFCERFTExec
	} else if entryType == IFCERFTExec && !ex {
		expectedType = IFCERFTFile
	} else {
...
func TestSetExFileSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTFile, true)
}

func TestSetNoExFileSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTFile, false)
}

func TestSetExExecSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTExec, true)
}

func TestSetNoExExecSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTExec, false)
}

func TestSetExDirSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTDir, true)
}

func TestSetNoExDirSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTDir, false)
}

func TestSetExSymSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTSym, true)
}

func TestSetNoExSymSuccess(t *testing.T) {
	testSetExSuccess(t, IFCERFTSym, false)
}
...
func TestSetExFailNoSuchName(t *testing.T) {
	...
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	expectedErr := IFCERFTNoSuchNameError{p.TailName()}
...
func makeBlockStateDirty(config IFCERFTConfig, rmd *IFCERFTRootMetadata, p IFCERFTPath, ptr IFCERFTBlockPointer) {
...
func testSyncDirtySuccess(t *testing.T, isUnmerged bool) {
	...
	// fsync a
	config.DirtyBlockCache().Put(aNode.IFCERFTBlockPointer, p.Branch, aBlock)
	makeBlockStateDirty(config, rmd, p, aNode.IFCERFTBlockPointer)
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	...
	fileUpdate := IFCERFTBlockUpdate{aNode.IFCERFTBlockPointer, newP.path[1].IFCERFTBlockPointer}
...
func expectSyncDirtyBlock(config *ConfigMock, rmd *IFCERFTRootMetadata, p IFCERFTPath, ptr IFCERFTBlockPointer, block *FileBlock, splitAt int64,
	...
	newPtr := IFCERFTBlockPointer{ID: newID}
...
func TestSyncDirtyMultiBlocksSuccess(t *testing.T) {
	...
	config.DirtyBlockCache().Put(fileNode.IFCERFTBlockPointer, p.Branch, fileBlock)
	makeBlockStateDirty(config, rmd, p, fileNode.IFCERFTBlockPointer)
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	...
	expectSyncDirtyBlock(config, rmd, p, fileBlock.IPtrs[3].IFCERFTBlockPointer,
	...
	refBlocks := []IFCERFTBlockPointer{fileBlock.IPtrs[1].IFCERFTBlockPointer,
		fileBlock.IPtrs[3].IFCERFTBlockPointer}
	unrefBlocks := []IFCERFTBlockPointer{
	...
	fileUpdate := IFCERFTBlockUpdate{fileNode.IFCERFTBlockPointer, newP.path[1].IFCERFTBlockPointer}
	if so.File != fileUpdate {
		t.Errorf("Incorrect file update in op: %v vs. %v", so.File,
			fileUpdate)
	}
...
func TestSyncDirtyDupBlockSuccess(t *testing.T) {
	...
	aNode := IFCERFTPathNode{makeBP(aID, rmd, config, uid), "a"}
	bNode := IFCERFTPathNode{makeBP(bID, rmd, config, uid), "b"}
	p := IFCERFTPath{IFCERFTFolderBranch{Tlf: id}, []IFCERFTPathNode{node, bNode}}
	ops := getOps(config, id)
	n := nodeFromPath(t, ops, p)
	...
	config.DirtyBlockCache().Put(bNode.IFCERFTBlockPointer, p.Branch, bBlock)
	makeBlockStateDirty(config, rmd, p, bNode.IFCERFTBlockPointer)
	testPutBlockInCache(config, node.IFCERFTBlockPointer, id, rootBlock)
	testPutBlockInCache(config, aNode.IFCERFTBlockPointer, id, aBlock)
	...
	refNonce := IFCERFTBlockRefNonce{1}
	...
	expectedPath.path = append(expectedPath.path,
		IFCERFTPathNode{IFCERFTBlockPointer{ID: aID, IFCERFTBlockContext: IFCERFTBlockContext{RefNonce: refNonce}}, "b"})
	config.mockBops.EXPECT().Put(gomock.Any(), rmdMatcher{rmd},
		ptrMatcher{expectedPath.path[1].IFCERFTBlockPointer}, readyBlockData).
		Return(nil)
	...
	checkOp(t, so.OpCommon, nil, nil, updates)
	fileUpdate := IFCERFTBlockUpdate{bNode.IFCERFTBlockPointer, newP.path[1].IFCERFTBlockPointer}
	if so.File != fileUpdate {
		t.Errorf("Incorrect file update in op: %v vs. %v", so.File,
			fileUpdate)
	}
...
func putAndCleanAnyBlock(config *ConfigMock, p IFCERFTPath) {
	config.mockBcache.EXPECT().Put(gomock.Any(), gomock.Any(), gomock.Any(), IFCERFTTransientEntry).
		Do(func(ptr IFCERFTBlockPointer, tlf IFCERFTTlfID, block IFCERFTBlock, lifetime IFCERFTBlockCacheLifetime) {
			config.mockDirtyBcache.EXPECT().
				Get(ptrMatcher{IFCERFTBlockPointer{ID: ptr.ID}}, p.Branch).
				AnyTimes().Return(nil, IFCERFTNoSuchBlockError{ptr.ID})
			config.mockBcache.EXPECT().
				Get(ptrMatcher{IFCERFTBlockPointer{ID: ptr.ID}}).
				AnyTimes().Return(block, nil)
		}).AnyTimes().Return(nil)
...
func TestSyncDirtyMultiBlocksSplitInBlockSuccess(t *testing.T) {
	...
	config.mockDirtyBcache.EXPECT().IsDirty(
		ptrMatcher{fileBlock.IPtrs[0].IFCERFTBlockPointer},
		p.Branch).AnyTimes().Return(false)
	makeBlockStateDirty(config, rmd, p, fileNode.IFCERFTBlockPointer)
	config.mockDirtyBcache.EXPECT().IsDirty(
		ptrMatcher{fileBlock.IPtrs[2].IFCERFTBlockPointer},
		p.Branch).Return(false)
	config.mockDirtyBcache.EXPECT().Get(ptrMatcher{fileBlock.IPtrs[2].IFCERFTBlockPointer},
		p.Branch).Return(nil,
		IFCERFTNoSuchBlockError{fileBlock.IPtrs[2].IFCERFTBlockPointer.ID})
	config.mockBcache.EXPECT().Get(ptrMatcher{fileBlock.IPtrs[2].IFCERFTBlockPointer}).
		Return(block3, nil)
	config.mockDirtyBcache.EXPECT().IsDirty(
		ptrMatcher{fileBlock.IPtrs[3].IFCERFTBlockPointer},
		p.Branch).AnyTimes().Return(false)
	config.mockDirtyBcache.EXPECT().IsDirty(ptrMatcher{node.IFCERFTBlockPointer},
		p.Branch).AnyTimes().Return(true)
	makeBlockStateDirty(config, rmd, p, node.IFCERFTBlockPointer)
	config.mockDirtyBcache.EXPECT().Get(ptrMatcher{node.IFCERFTBlockPointer}, p.Branch).
		AnyTimes().Return(rootBlock, nil)
	config.mockDirtyBcache.EXPECT().IsDirty(ptrMatcher{fileNode.IFCERFTBlockPointer},
		p.Branch).AnyTimes().Return(true)
	config.mockDirtyBcache.EXPECT().Get(ptrMatcher{fileNode.IFCERFTBlockPointer},
	...
	config.mockBcache.EXPECT().CheckForKnownPtr(gomock.Any(), gomock.Any()).
		AnyTimes().Return(IFCERFTBlockPointer{}, nil)
	...
	config.mockDirtyBcache.EXPECT().Put(fileNode.IFCERFTBlockPointer, p.Branch,
		gomock.Any()).AnyTimes().Return(nil)
...
func TestSyncDirtyMultiBlocksCopyNextBlockSuccess(t *testing.T) {
	...
	config.mockDirtyBcache.EXPECT().Get(ptrMatcher{node.IFCERFTBlockPointer}, p.Branch).
		AnyTimes().Return(rootBlock, nil)
	config.mockDirtyBcache.EXPECT().Get(ptrMatcher{fileBlock.IPtrs[1].IFCERFTBlockPointer},
		p.Branch).Return(nil,
		IFCERFTNoSuchBlockError{fileBlock.IPtrs[1].IFCERFTBlockPointer.ID})
	config.mockBcache.EXPECT().Get(ptrMatcher{fileBlock.IPtrs[1].IFCERFTBlockPointer}).
		Return(block2, nil)
	config.mockDirtyBcache.EXPECT().IsDirty(
		ptrMatcher{fileBlock.IPtrs[1].IFCERFTBlockPointer},
		p.Branch).AnyTimes().Return(false)
	config.mockDirtyBcache.EXPECT().Get(ptrMatcher{fileBlock.IPtrs[3].IFCERFTBlockPointer},
		p.Branch).Return(nil,
		IFCERFTNoSuchBlockError{fileBlock.IPtrs[3].IFCERFTBlockPointer.ID})
	config.mockBcache.EXPECT().Get(ptrMatcher{fileBlock.IPtrs[3].IFCERFTBlockPointer}).
		Return(block4, nil)
	config.mockDirtyBcache.EXPECT().IsDirty(
		ptrMatcher{fileBlock.IPtrs[3].IFCERFTBlockPointer},
		p.Branch).Return(false)
	...
	// the split is in the middle
	pad1 := 14
	expectSyncDirtyBlock(config, rmd, p, fileBlock.IPtrs[0].IFCERFTBlockPointer,
	...
	split4At := int64(3)
	pad4 := 15
	expectSyncDirtyBlock(config, rmd, p, fileBlock.IPtrs[2].IFCERFTBlockPointer,
...
func TestSyncDirtyWithBlockChangePointerSuccess(t *testing.T) {
	...
	changePlainSize := 1
	changeBuf := []byte{253}
	changeReadyBlockData := IFCERFTReadyBlockData{
		buf: changeBuf,
	}
	...
	config.mockBops.EXPECT().Put(gomock.Any(), rmdMatcher{rmd},
		ptrMatcher{IFCERFTBlockPointer{ID: changeBlockID}}, changeReadyBlockData).
		Return(nil)
...
func TestKBFSOpsFailingRootOps(t *testing.T) {
	...
	if _, ok := err.(IFCERFTInvalidParentPathError); !ok {
...
func TestKBFSOpsBackgroundFlush(t *testing.T) {
	...
	config.Notifier().RegisterForChanges([]IFCERFTFolderBranch{{id, IFCERFTMasterBranch}},
		observer)
...
func TestKBFSOpsFailToReadUnverifiableBlock(t *testing.T) {
	...
	if _, ok := err.(IFCERFTHashMismatchError); !ok {
...
func (c cryptoFixedTlf) MakeRandomTlfID(isPublic bool) (IFCERFTTlfID, error) {
	return c.tlf, nil
}
...
func TestKBFSOpsMaliciousMDServerRange(t *testing.T) {
	...
	_, _, err := kbfsOps1.CreateFile(ctx, rootNode1, "dummy.txt", false, IFCERFTNoEXCL)
	require.NoError(t, err)
	...
	_, _, err = kbfsOps2.CreateFile(
		ctx, rootNode2, "dummy.txt", false, IFCERFTNoEXCL)
	require.NoError(t, err)
	...
	require.IsType(t, IFCERFTMDPrevRootMismatch{}, err)
}
