...
func testCreateEntrySuccess(t *testing.T, entryType EntryType) {
	...
	switch entryType {
	case File:
		newN, _, err = config.KBFSOps().CreateFile(ctx, n, "b", false, false)
	case Exec:
		newN, _, err = config.KBFSOps().CreateFile(ctx, n, "b", true, false)
	case Dir:
...
func testCreateEntryFailKBFSPrefix(t *testing.T, et EntryType) {
	...
	case Exec:
		_, _, err = config.KBFSOps().CreateFile(ctx, n, name, true, false)
	case File:
		_, _, err = config.KBFSOps().CreateFile(ctx, n, name, false, false)
	}
	if err == nil {
		t.Errorf("Got no expected error on create")
	} else if err != expectedErr {
...
func TestKBFSOpsWriteRenameStat(t *testing.T) {
	...
	fileNode, _, err := kbfsOps.CreateFile(ctx, rootNode, "a", false, false)
	if err != nil {
...
func TestKBFSOpsCreateFileWithArchivedBlock(t *testing.T) {
	...
	kbfsOps := config.KBFSOps()
	_, _, err := kbfsOps.CreateFile(ctx, rootNode, "a", false, false)
	if err != nil {
		t.Fatalf("Couldn't create file: %v", err)
	}
	...
	// forced to try again.
	_, _, err = kbfsOps.CreateFile(ctx, rootNode, "b", false, false)
	if err != nil {
...
func TestKBFSOpsMaliciousMDServerRange(t *testing.T) {
	...
	_, _, err := kbfsOps1.CreateFile(ctx, rootNode1, "dummy.txt", false, false)
	require.NoError(t, err)
	...
	_, _, err = kbfsOps2.CreateFile(
		ctx, rootNode2, "dummy.txt", false, false)
	require.NoError(t, err)
