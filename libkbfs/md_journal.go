...
func MakeImmutableBareRootMetadata(
	rmd BareRootMetadata, extra ExtraMetadata, mdID tlf.MdID,
...
func (j mdJournal) mdPath(id tlf.MdID) string {
	idStr := id.String()
	return filepath.Join(j.mdsPath(), idStr[:4], idStr[4:34])
}

func (j mdJournal) mdDataPath(id tlf.MdID) string {
	return filepath.Join(j.mdPath(id), "data")
}

func (j mdJournal) mdInfoPath(id tlf.MdID) string {
...
func (j mdJournal) getMDInfo(id tlf.MdID) (time.Time, MetadataVer, error) {
	var info mdInfo
...
func (j mdJournal) putMDInfo(
	id tlf.MdID, timestamp time.Time, version MetadataVer) error {
...
func (j mdJournal) putMD(rmd BareRootMetadata) (tlf.MdID, error) {
...
func (j *mdJournal) removeMD(id tlf.MdID) error {
	path := j.mdPath(id)
...
func (j mdJournal) getEarliestWithExtra(verifyBranchID bool) (
	tlf.MdID, MutableBareRootMetadata, ExtraMetadata, time.Time, error) {
	entry, exists, err := j.j.getEarliestEntry()
	if err != nil {
		return tlf.MdID{}, nil, nil, time.Time{}, err
	}
	if !exists {
		return tlf.MdID{}, nil, nil, time.Time{}, nil
	}
...
func (j *mdJournal) convertToBranch(
	...
	mdsToRemove := make([]tlf.MdID, 0, len(allEntries))
	defer func() {
...
func (j mdJournal) getNextEntryToFlush(
	...
	tlf.MdID, *RootMetadataSigned, ExtraMetadata, error) {
	mdID, rmd, extra, timestamp, err := j.getEarliestWithExtra(true)
	if err != nil {
		return tlf.MdID{}, nil, nil, err
	}
	if rmd == nil || rmd.RevisionNumber() >= end {
		return tlf.MdID{}, nil, nil, nil
	}
...
func (j *mdJournal) removeFlushedEntry(
	ctx context.Context, mdID tlf.MdID, rmds *RootMetadataSigned) (
...
func (j *mdJournal) put(
	...
	if mStatus == Merged && j.branchID != NullBranchID {
		return tlf.MdID{}, MDJournalConflictError{}
	}
	...
	if isLocalSquash && rmd.BID() != NullBranchID {
		return tlf.MdID{}, errors.Errorf("A local squash must have a null branch ID,"+
	...
	if head != (ImmutableBareRootMetadata{}) {
		...
		if !ok {
			// TODO: Use a non-server error.
			return tlf.MdID{}, MDServerErrorUnauthorized{}
		}
		...
		if isLocalSquash {
			...
			if exists && !entry.IsLocalSquash {
				return tlf.MdID{}, errors.Errorf("Local squash is not preceded "+
...
func (j *mdJournal) resolveAndClear(
	...
	if rmd.BID() != NullBranchID {
		return tlf.MdID{}, errors.Errorf("Resolution MD has branch ID: %s", rmd.BID())
	}

	// The branch ID must match our current state.
	if bid == NullBranchID {
		return tlf.MdID{}, errors.New("Cannot resolve master branch")
	}
	if j.branchID != bid {
		return tlf.MdID{}, errors.Errorf("Resolve and clear for branch %s "+
