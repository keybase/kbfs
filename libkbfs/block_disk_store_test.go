...
func testBlockDiskStoreGCd(t *testing.T, j *blockDiskStore) {
	filepath.Walk(j.dir,
...
func TestBlockDiskStoreFlush(t *testing.T) {
	ctx, tempdir, j := setupBlockDiskStoreTest(t)
	defer teardownBlockDiskStoreTest(t, tempdir, j)
	...
	bCtx2 := addBlockDiskRef(ctx, t, j, bID)
	bCtx3 := addBlockDiskRef(ctx, t, j, bID)
	...
	flush := func() {
		...
		var partialEntries blockDiskEntriesToFlush
	...
	testBlockDiskStoreGCd(t, j)
}

func TestBlockDiskStoreFlushInterleaved(t *testing.T) {
	ctx, tempdir, j := setupBlockDiskStoreTest(t)
	defer teardownBlockDiskStoreTest(t, tempdir, j)
	...
	flushOne := func() {
		...
		err = flushBlockDiskEntries(ctx, j.log, blockServer,
...
func TestBlockDiskStoreFlushMDRevMarker(t *testing.T) {
	ctx, tempdir, j := setupBlockDiskStoreTest(t)
	defer teardownBlockDiskStoreTest(t, tempdir, j)
...
func TestBlockDiskStoreIgnoreBlocks(t *testing.T) {
	ctx, tempdir, j := setupBlockDiskStoreTest(t)
	defer teardownBlockDiskStoreTest(t, tempdir, j)

	// Put a few blocks
	data1 := []byte{1, 2, 3, 4}
	bID1, _, _ := putBlockDiskData(ctx, t, j, data1)
	data2 := []byte{5, 6, 7, 8}
	bID2, _, _ := putBlockDiskData(ctx, t, j, data2)
	...
	bID3, _, _ := putBlockDiskData(ctx, t, j, data3)
	data4 := []byte{13, 14, 15, 16}
	bID4, _, _ := putBlockDiskData(ctx, t, j, data4)
...
func TestBlockDiskStoreSaveUntilMDFlush(t *testing.T) {
	ctx, tempdir, j := setupBlockDiskStoreTest(t)
	defer teardownBlockDiskStoreTest(t, tempdir, j)
	...
	bID5, _, _ := putBlockDiskData(ctx, t, j, data5)
	data6 := []byte{21, 22, 23, 24}
	bID6, _, _ := putBlockDiskData(ctx, t, j, data6)
	err = j.saveBlocksUntilNextMDFlush()
	require.NoError(t, err)
...

	{
		...
		jRestarted, err := makeBlockDiskStore(
