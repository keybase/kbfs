...
func testPseudonym(t *testing.T, name, idStr string, keyGen keyGen,
	...
	pseudonym, err := MakePseudonym(name, id, keyGen, key)
	require.NoError(t, err)
			...
			for _, keyGen := range keyGens {
				for _, keyStr := range keyStrs {
					...
					t.Run(testPseudonymName(name, idStr, keyGen, keyStr, epStr),
						func(t *testing.T) {
							testPseudonym(t, name, idStr, keyGen, keyStr, epStr)
						})
					i++
				}
			}
