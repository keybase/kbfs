...
func (k *LibKBFS) GetUID(u User) (uid keybase1.UID) {
	config, ok := u.(libkbfs.IFCERFTConfig)
	if !ok {
...
func (k *LibKBFS) GetRootDir(u User, tlfName string, isPublic bool, expectedCanonicalTlfName string) (
	...
	k.refs[config][dir.(libkbfs.IFCERFTNode)] = true
	return dir, nil
}
...
func (k *LibKBFS) CreateDir(u User, parentDir Node, name string) (dir Node, err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	dir, _, err = kbfsOps.CreateDir(ctx, parentDir.(libkbfs.IFCERFTNode), name)
	if err != nil {
		return dir, err
	}
	k.refs[config][dir.(libkbfs.IFCERFTNode)] = true
...
func (k *LibKBFS) CreateLink(u User, parentDir Node, fromName, toPath string) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	_, err = kbfsOps.CreateLink(ctx, parentDir.(libkbfs.IFCERFTNode), fromName, toPath)
	return err
}
...
func (k *LibKBFS) RemoveDir(u User, dir Node, name string) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	return kbfsOps.RemoveDir(ctx, dir.(libkbfs.IFCERFTNode), name)
}
...
func (k *LibKBFS) RemoveEntry(u User, dir Node, name string) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	return kbfsOps.RemoveEntry(ctx, dir.(libkbfs.IFCERFTNode), name)
}
...
func (k *LibKBFS) Rename(u User, srcDir Node, srcName string,
	...
	ctx, cancel := k.newContext()
	defer cancel()
	return kbfsOps.Rename(ctx, srcDir.(libkbfs.IFCERFTNode), srcName, dstDir.(libkbfs.IFCERFTNode), dstName)
}
...
func (k *LibKBFS) WriteFile(u User, file Node, data []byte, off int64, sync bool) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	err = kbfsOps.Write(ctx, file.(libkbfs.IFCERFTNode), data, off)
	if err != nil {
		return err
	}
	if sync {
		ctx, cancel := k.newContext()
		defer cancel()
		err = kbfsOps.Sync(ctx, file.(libkbfs.IFCERFTNode))
	}
	return err
}
...
func (k *LibKBFS) TruncateFile(u User, file Node, size uint64, sync bool) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	err = kbfsOps.Truncate(ctx, file.(libkbfs.IFCERFTNode), size)
	if err != nil {
		return err
	}
	if sync {
...
func (k *LibKBFS) Sync(u User, file Node) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	return kbfsOps.Sync(ctx, file.(libkbfs.IFCERFTNode))
}
...
func (k *LibKBFS) ReadFile(u User, file Node, off int64, buf []byte) (length int, err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	numRead, err = kbfsOps.Read(ctx, file.(libkbfs.IFCERFTNode), buf, off)
	if err != nil {
		return 0, err
	}
...
func (k *LibKBFS) Lookup(u User, parentDir Node, name string) (file Node, symPath string, err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	file, ei, err := kbfsOps.Lookup(ctx, parentDir.(libkbfs.IFCERFTNode), name)
	if err != nil {
...
func (k *LibKBFS) SetEx(u User, file Node, ex bool) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	return kbfsOps.SetEx(ctx, file.(libkbfs.IFCERFTNode), ex)
}
...
func (k *LibKBFS) SetMtime(u User, file Node, mtime time.Time) (err error) {
	...
	ctx, cancel := k.newContext()
	defer cancel()
	return kbfsOps.SetMtime(ctx, file.(libkbfs.IFCERFTNode), &mtime)
}
...
func (k *LibKBFS) GetMtime(u User, file Node) (mtime time.Time, err error) {
	...
	if node, ok := file.(libkbfs.IFCERFTNode); ok {
		...
		_, info, err = kbfsOps.Lookup(ctx, node.parentDir.(libkbfs.IFCERFTNode),
			node.name)
	}
	if err != nil {
...
// name.
func getRootNode(ctx context.Context, config libkbfs.IFCERFTConfig, tlfName string,
	isPublic bool) (libkbfs.IFCERFTNode, error) {
	h, err := parseTlfHandle(ctx, config.KBPKI(), tlfName, isPublic)
	if err != nil {
		return nil, err
	}
