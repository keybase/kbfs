...
type fileOpener interface {
	open(ctx context.Context, oc *openContext, path []string) (f cbfs.File, isDir bool, err error)
	cbfs.File
}
...
func (*FolderList) GetFileInformation(context.Context) (*cbfs.Stat, error) {
...
func (fl *FolderList) open(ctx context.Context, oc *openContext, path []string) (f cbfs.File, isDir bool, err error) {
	...
	for oc.reduceRedirectionsLeft() {
		...
		case libkbfs.TlfNameNotCanonical:
			...
			if len(path) == 1 && oc.isOpenReparsePoint() {
				// TODO handle dir/non-dir here, semantics?
				fl.fs.log.CDebugf(ctx, "FL Lookup ret alias")
				return &Alias{canon: aliasTarget}, true, nil
			}
			path[0] = aliasTarget
			continue
...
// FindFiles for cbfs.
func (fl *FolderList) FindFiles(ctx context.Context, ignored string, callback func(*cbfs.NamedStat) error) (err error) {
