...
type Folder struct {
	...
	// FUSE Forget request.
	nodes map[libkbfs.NodeID]cbfs.File
...
func (f *Folder) lockedAddNode(node libkbfs.Node, val cbfs.File) {
	f.mu.Lock()
...
func (d *Dir) GetFileInformation(ctx context.Context) (st *cbfs.Stat, err error) {
...
func (d *Dir) SetFileAttributes(ctx context.Context, fileAttributes *cbfs.Stat) error {
...
func (d *Dir) open(ctx context.Context, oc *openContext, path []string) (cbfs.File, bool, error) {
	...
	for len(path) > 0 {
		...
		if c := lowerTranslateCandidate(oc, path[0]); c != "" {
			var hit string
			var nhits int
			d.FindFiles(ctx, c, func(ns *cbfs.NamedStat) error {
		...
		if leaf {
			...
			case !notFound && oc.isExistingError():
				return nil, false, cbfs.ErrFileAlreadyExists
			}
		}
...
func openFile(ctx context.Context, oc *openContext, path []string, f *File) (cbfs.File, bool, error) {
	var err error
...
func openSymlink(ctx context.Context, oc *openContext, parent *Dir, rootDir *Dir, origPath, path []string, target string) (cbfs.File, bool, error) {
	...
	if target == "" || target[0] == '/' {
		return nil, false, cbfs.ErrNotSupported
	}
...
func getExclFromOpenContext(oc *openContext) libkbfs.Excl {
	return libkbfs.Excl(false) // FIXME libkbfs.Excl(oc.CreateDisposition == cbfs.FileCreate)
}

func (d *Dir) create(ctx context.Context, oc *openContext, name string) (f cbfs.File, isDir bool, err error) {
...
func (d *Dir) FindFiles(ctx context.Context, ignored string, callback func(*cbfs.NamedStat) error) (err error) {
...
func (d *Dir) CanDelete(ctx context.Context) (err error) {
	d.folder.fs.logEnterf(ctx, "Dir CanDelete %q", d.name)
	defer func() { d.folder.reportErr(ctx, libkbfs.WriteMode, err) }()
	...
	if len(children) > 0 {
		return cbfs.ErrDirectoryNotEmpty
	}

	return nil
}

func (d *Dir) Delete(ctx context.Context) (err error) {
	d.folder.fs.logEnterf(ctx, "Dir Delete %q", d.name)
	defer func() { d.folder.reportErr(ctx, libkbfs.WriteMode, err) }()
	if d.parent != nil {
		...
		return d.folder.fs.config.KBFSOps().RemoveDir(ctx, d.parent, d.name)
	}
	return nil
}
...
func (d *Dir) Cleanup(ctx context.Context) {
...
func (d *Dir) IsDirectoryEmpty(ctx context.Context) (bool, error) {
	d.folder.fs.logEnterf(ctx, "Dir IsFolderEmpty %q", d.name)
	children, err := d.folder.fs.config.KBFSOps().GetDirChildren(ctx, d.node)
	if err != nil {
	...
	for _, p := range strings.FieldsFunc(targetPath, isPathSeparator) {
		...
		case "..":
			if len(pathComponents) == 0 {
				return nil, cbfs.ErrNotSupported
			}
...
func asDir(ctx context.Context, f cbfs.File) *Dir {
	switch x := f.(type) {
	case *Dir:
		return x
	case *TLF:
