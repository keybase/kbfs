...
func Start(mounter Mounter, options StartOptions, kbCtx libkbfs.Context) *libfs.Error {
	...
	options.CbfsConfig.FileSystem = fs
	options.CbfsConfig.Path = mounter.Dir()
	if options.CbfsConfig.Path == "" {
		// The mounter will detect this case and pick up the path from CbfsConfig
		options.CbfsConfig.Path, err = config.KeybaseService().EstablishMountDir(ctx)
		if err != nil {
			return libfs.InitError(err.Error())
		}
		log.CInfof(ctx, "Got mount dir from service: %s", options.CbfsConfig.Path)
	}
	...
	if !strings.EqualFold(options.CbfsConfig.Path, "none") {
		err = mounter.Mount(&options.CbfsConfig, log)
		if err != nil {
