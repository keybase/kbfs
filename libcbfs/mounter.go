...
type Mounter interface {
	Dir() string
	Mount(*cbfs.Config, logger.Logger) error
	Unmount() error
}
...
func (m *DefaultMounter) Mount(cfg *cbfs.Config, log logger.Logger) error {
	var err error
	var h *cbfs.MountHandle
	...
	//	break
	//}
	//log.Errorf("Failed to mount cbfs filesystem (i=%d): %v", i, err)
...
// go-routine.
func (m *DefaultMounter) mountHelper(cfg *cbfs.Config) (*cbfs.MountHandle, error) {
	// m.dir is constant and safe to access outside the lock.
	handle, err := cbfs.Mount(cfg)
	if err != nil {
		return nil, err
	}
...
// called.
func (m NoopMounter) Mount(_ *cbfs.Config, _ logger.Logger) error {
	<-m.c
	return nil
}
